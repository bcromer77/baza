...
router.post('/', async (req, res) => {
  try {
    ...
    if (!creatorId || !videoUrls || !Array.isArray(videoUrls) || videoUrls.length === 0) {
      return res.status(400).json({ success: false, message: 'Missing creatorId or video URLs' });
    }

    const videoUrl = videoUrls[0]; // Use first for now
    const outputPath = path.join(__dirname, '../../temp_audio.mp3');

    console.log(`🎬 Running transcription for: ${videoUrl}`);

    // Spawn Python process to transcribe
    const python = spawn('python3', ['scripts/transcribe.py', videoUrl]);
    ...
    python.on('close', async (code) => {
      ...
      if (code !== 0 || !transcriptData) {
        return res.status(500).json({ success: false, message: 'Transcription failed', error: errorData });
      }
      ...
      // Clean temp audio if it exists
      if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath);

      res.status(200).json({ success: true, transcript: transcriptData, lines });
    });
