...
export default function CreatorDashboard() {
  ...
  const [query, setQuery] = useState({ location: '', topic: '', sentiment: '', minEngagement: '' });
  ...
  const handleQuerySubmit = async (e: any) => {
    e.preventDefault();
    if (!creatorId) return;
    const response = await fetch(`http://localhost:5000/api/creators/${creatorId}/query`, {
      method: 'POST',
  ...
  if (!dashboardData) return <div className="text-white p-10">Loading dashboard...</div>;

  return (
    <div className="min-h-screen bg-black text-white p-6">
      <h1 className="text-4xl font-bold mb-6 text-yellow-400 text-center">Creator Prism Dashboard</h1>
      ...
      <section className="mb-10">
        ...
        {dashboardData.audienceInsights.map((insight, index) => (
          <div key={index} className="bg-gray-900 p-4 rounded-lg mb-4">
            <p>📍 Location: {Object.entries(insight.geography)[0]?.join(': ')}%</p>
            <p>🎯 Intent: {Object.entries(insight.intent)[0]?.join(': ')}%</p>
            <p>📊 Behavior: {Object.entries(insight.behavior)[0]?.join(': ')}%</p>
            <p>⏰ Engagement: {insight.engagementTimes.peak}</p>
            <p>🧠 Sentiment:</p>
            <ul className="ml-4">
              {Object.entries(insight.sentiment).map(([topic, data]) => (
                <li key={topic}>
                  {topic}: {data.sentiment} (score {data.score.toFixed(2)})
                </li>
              ))}
            </ul>
          </div>
      ...
      {/* 🔥 Hot Opportunities */}
      <section className="mb-10">
        <h2 className="text-2xl font-semibold text-yellow-400 mb-4">🔥 Hot Opportunities</h2>
      ...
      <section className="mb-10">
        ...
        {queryResult && (
          <div className="mt-4 bg-gray-900 p-4 rounded-lg">
            <p>
              🎯 <strong>{queryResult.matches}</strong> out of <strong>{queryResult.totalAudience}</strong> followers matched
            </p>
          </div>
        )}
      </section>
      ...
      {/* 🧠 Smart Search */}
      <section className="mb-10">
        <h2 className="text-2xl font-semibold mb-2 text-yellow-400">🧠 Smart Search</h2>
        <input
          type="text"
          className="w-full p-2 text-black rounded"
          placeholder="Try: Show me creators in Portugal talking about Ozempic"
          value={prompt}
        ...
        {brandResults.length > 0 && (
          <div className="mt-4">
            <h3 className="text-xl mb-2 text-white">Results:</h3>
      ...
      {/* 🤝 Brand Offers */}
      <section>
        <h2 className="text-2xl font-semibold mb-4">Brand Offers</h2>
        {dashboardData.brandOffers.map(offer => (
          <div key={offer.id} className="bg-gray-800 p-4 rounded-lg mb-2">
            <p className="text-yellow-400">{offer.brand}</p>
            <p>{offer.offer}</p>
          </div>
        ))}
      </section>
