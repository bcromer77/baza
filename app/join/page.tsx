...
import { Mic, Headphones, ArrowRight, CheckCircle } from "lucide-react"
  ...
  const handleSignup = async () => {
    ...
    try {
      const res = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ name, email, accountType: "creator" }),
      })
      const data = await res.json()
      if (data.success) {
        // Start hidden Phyllo flow in the background (fire and forget)
        fetch("/api/phyllo/initiate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId: data.userId }),
        })

        // Trigger Stripe Connect OAuth (opens new tab)
        window.open(`/api/stripe/connect?userId=${data.userId}`, "_blank")
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10">
        ...
        <div className="absolute top-1/2 -right-40 w-80 h-80 bg-purple-500/20 rounded-full blur-3xl" />
          ...
          <Card className="w-full bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 shadow-xl overflow-hidden">
            ...
            <CardContent className="p-8 space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-zinc-400">Join Audiantix and turn your voice into travel partnerships and income.</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-500 hover:opacity-90 text-black font-medium h-11"
                >
              ...
              <div className="text-xs text-zinc-500 space-y-2">
                <p>By joining, you'll get access to our creator tools, analytics, and partnership opportunities.</p>
                <p>We'll never share your info. Audiantix is built for creators.</p>
              </div>
            </CardContent>
          </Card>
        ) : (
